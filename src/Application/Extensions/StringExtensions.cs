...
namespace Application.Extensions
{
    ...
    public static class StringExtensions
    {
        ...
        /// <summary>
        /// Xoá dâu câu khỏi chuỗi: vd tôi -> toi
        /// </summary>
        public static string RemoveDiacriticsFromString(string text, bool removeSpace)
        {
            ...
            var desiredText = stringBuilder.ToString().Normalize(NormalizationForm.FormC);

            return !removeSpace ? desiredText : desiredText.Replace(" ", "");
        }

        public static string GetLast8DigitPhoneNumber(string phone)
        {
            const int digit = 8;
            return phone.Length >= digit
                ? phone.Substring(phone.Length - digit)
                : phone;
        }

        public static string GetAllFirstCharacterOfString(string text)
        {
            var normalizedString = RemoveDiacriticsFromString(text, false);
            var firstChars = normalizedString.Where((ch, index) =>
                ch != ' ' && (index == 0 || normalizedString[index - 1] == ' '));

            var result = string.Join("", firstChars);
            return result;
        }
    }
}