...
namespace WebApi.Controllers.client
{
    ...
    /// </summary>
    [Route("api/auth/")]
    public class TokenAuthController : BaseNonAuthClientApiController
    {
        ...
        public async Task<AjaxResponse<AuthenticateResultModel>> Authenticate([FromBody] AuthenticateModel model)
        {
            try
            {
                var token = await _jWtAuthenticationManager.Authenticate(model.UserName, model.Password, false);
        ...
        {
            var token = await _tokenRefresher.Refresh(refresh, false);
